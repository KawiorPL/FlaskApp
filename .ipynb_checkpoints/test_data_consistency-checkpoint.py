...
DB_FILE = 'DbOksary.db'  # Upewnij się, że to poprawna ścieżka
...
class TestDataConsistency:
    ...
    def test_aktorzy(self):
        ...
        assert len(csv_aktorzy) == len(db_aktorzy_df), "Liczba aktorów w CSV i DB jest różna"
        assert all(aktor in db_aktorzy_df['imie_nazwisko'].values for aktor in csv_aktorzy), "Aktorzy w CSV i DB się nie zgadzają"
    ...
    def test_filmy(self):
        ...
        assert len(csv_filmy) == len(db_filmy_df), "Liczba filmów w CSV i DB jest różna"
        assert all(film in db_filmy_df['film'].values for film in csv_filmy), "Filmy w CSV i DB się nie zgadzają"
    ...
    def test_typy_nagrod(self):
        ...
        assert len(csv_typy_nagrod) == len(db_typy_nagrod_df), "Liczba typów nagród w CSV i DB jest różna"
        assert all(typ in db_typy_nagrod_df['nazwa_typu'].values for typ in csv_typy_nagrod), "Typy nagród w CSV i DB się nie zgadzają"
    ...
    def test_nagrody_rok(self):
        ...
        assert len(csv_lata) == len(db_lata_df), "Liczba lat w CSV i DB jest różna"
        assert all(rok in db_lata_df['rok'].values for rok in csv_lata), "Lata w CSV i DB się nie zgadzają"
