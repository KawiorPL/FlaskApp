...
class TestDataConsistency:
    ...
    def test_filmy(self):
        """Test porównujący liczbę filmów z CSV i bazy danych."""
        query = "SELECT DISTINCT film FROM nagrody WHERE film != ''"
        db_filmy_df = get_data_from_db(DB_FILE, query)
        csv_filmy = self.csv_data["film"].unique()
        csv_filmy = [re.sub(r'[""]', '', str(film)).strip().lower() for film in csv_filmy if pd.notna(film)]
